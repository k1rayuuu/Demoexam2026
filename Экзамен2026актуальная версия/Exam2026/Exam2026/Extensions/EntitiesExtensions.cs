...
namespace Exam2026.Extensions
{
    public static class EntitiesExtensions
    {
        ...
        public static OrderDto ToDto(this Orders order)
        {
            ...
            {
                Id = order.Id,
                Адрес_пункта_выдачи = order.Адрес_пункта_выдачи,
                Номер_заказа = order.Номер_заказа,
                Артикул_заказа = order.Артикул_заказа,
                Дата_заказа = order.Дата_заказа,
                Дата_доставки = order.Дата_доставки,
                ФИО_авторизированного_клиента = order.ФИО_авторизированного_клиента,
                Код_для_получения = order.Код_для_получения,
                Статус_заказа = order.Статус_заказа,
            };
        }
